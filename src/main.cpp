...
#if defined(NDEBUG)
# error "Instantcoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Instantcoin Signed Message:\n";
...
int64_t nStartSubsidy = 1 * COIN;
int64_t nMinSubsidy = .00001 * COIN;
...
static const int64_t nTargetTimespan = 1 * 60; // 1 minutes
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64_t TargetBlocksSpacingSeconds, uint64_t PastBlocksMin, uint64_t PastBlocksMax) {
        /* current difficulty formula reworked for Instantcoin */
