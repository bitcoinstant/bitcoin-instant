...
if [ -d "$OUT_DIR" -a -w "$OUT_DIR" ]; then
  set +e
  # Windows:
  cp /home/stallman/instantcoin/win32-build/src/bitcoind.exe $OUT_DIR/bitcoind.exe
  cp /home/stallman/instantcoin/win32-build/src/test/test_bitcoin.exe $OUT_DIR/test_bitcoin.exe
  cp /home/stallman/instantcoin/win32-build/src/qt/bitcoind-qt.exe $OUT_DIR/bitcoin-qt.exe
  # Linux:
  cp /home/stallman/instantcoin/linux-build/src/bitcoind $OUT_DIR/bitcoind
  cp /home/stallman/instantcoin/linux-build/src/test/test_bitcoin $OUT_DIR/test_bitcoin
  cp /home/stallman/instantcoin/linux-build/src/qt/bitcoind-qt $OUT_DIR/bitcoin-qt
  set -e
fi

# Run unit tests and blockchain-tester on Linux:
cd /home/stallman/instantcoin/linux-build
make check

# Run RPC integration test on Linux:
/home/stallman/instantcoin/qa/rpc-tests/wallet.sh /home/stallman/instantcoin/linux-build/src
/home/stallman/instantcoin/qa/rpc-tests/listtransactions.py --srcdir /home/stallman/instantcoin/linux-build/src
# Clean up cache/ directory that the python regression tests create
rm -rf cache

if [ $RUN_EXPENSIVE_TESTS = 1 ]; then
  # Run unit tests and blockchain-tester on Windows:
  cd /home/stallman/instantcoin/win32-build
  make check
fi
