...
*Setup instructions for a gitian build of Instantcoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Instantcoin
...
- [Building Instantcoin](#building-instantcoin)
...
Clone the git repositories for instantcoin and gitian,

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/instantcoin/instantcoin
```
...
In [doc/release-process.md](release-process.md) in the instantcoin repository under 'Fetch and build inputs'.
...
Building Instantcoin
----------------

To build Instantcoin (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the instantcoin repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/instantcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/instantcoin/instantcoin
...
For example:
```bash
URL=https://github.com/laanwj/instantcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit instantcoin=${COMMIT} --url instantcoin=${URL} ../instantcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit instantcoin=${COMMIT} --url instantcoin=${URL} ../instantcoin/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/instantcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/instantcoin-build.assert
```
...
[instantcoin/gitian.sigs](https://github.com/instantcoin/gitian.sigs/) repository, or
